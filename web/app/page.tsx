          ...
          <div className="w-full max-w-2xl mx-auto mb-12">
            ...
            <div className="flex items-center justify-between mb-3 px-1">
              ...
              {chatState.enableRag && (
                <select
                  ...
                  className="text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 outline-none focus:border-[#8DBF5A] dark:text-slate-200"
                >
                  {kbs.map((kb) => (
            ...
            <div className="relative">
              <input
                ref={inputRef}
                type="text"
                className="w-full px-5 py-4 pr-14 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#8DBF5A]/20 focus:border-[#8DBF5A] transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500 text-slate-700 dark:text-slate-200 shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50"
              ...
              <button
                ...
                className="absolute right-2 top-2 bottom-2 aspect-square bg-[#1a6b3a] text-white rounded-xl flex items-center justify-center hover:bg-[#155730] disabled:opacity-50 disabled:hover:bg-[#1a6b3a] transition-all shadow-md shadow-green-700/20"
              >
                {chatState.isLoading ? (
          ...
          <div className="flex items-center justify-between px-6 py-3 border-b border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-green-700 dark:hover:text-[#8DBF5A] hover:bg-green-50 dark:hover:bg-green-900/30 rounded-lg transition-colors"
          ...
          >
                  ...
                  <>
                    ...
                    <div className="flex-1 space-y-3">
                      <div className="bg-white dark:bg-slate-800 px-5 py-4 rounded-2xl rounded-tl-none border border-slate-200 dark:border-slate-700 shadow-sm">
                        ...
                        {/* Loading indicator */}
                        {msg.isStreaming && (
                          <div className="flex items-center gap-2 mt-3 text-green-700 dark:text-[#8DBF5A] text-sm">
                        ...
                        (msg.sources.rag?.length ?? 0) +
                          ...
                          <div className="flex flex-wrap gap-2">
                            {msg.sources.rag?.map((source, i) => (
                              <div
                                key={`rag-${i}`}
                                className="flex items-center gap-1.5 px-2.5 py-1 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-lg text-xs"
                              >
            ...
            {chatState.isLoading && chatState.currentStage && (
              <div className="flex gap-4 w-full max-w-4xl mx-auto">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#8DBF5A] to-green-700 flex items-center justify-center shrink-0">
          ...
          <div className="border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-6 py-4">
            <div className="max-w-4xl mx-auto relative">
              <input
                ref={inputRef}
                type="text"
                className="w-full px-5 py-3.5 pr-14 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#8DBF5A]/20 focus:border-[#8DBF5A] transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500 text-slate-700 dark:text-slate-200"
              ...
              <button
                onClick={handleSend}
                disabled={chatState.isLoading || !inputMessage.trim()}
                className="absolute right-2 top-2 bottom-2 aspect-square bg-[#1a6b3a] text-white rounded-lg flex items-center justify-center hover:bg-[#155730] disabled:opacity-50 disabled:hover:bg-[#1a6b3a] transition-all"
              >
                {chatState.isLoading ? (
                  <Loader2 className="w-5 h-5 animate-spin" />
                ) : (
